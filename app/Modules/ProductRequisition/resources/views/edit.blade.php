        ...
        <form action="{{route('admin.productrequisition.update',$requisition->id)}}" method="post">
            ...
            <div class="row">
                    ...
                    <div class="rs-table-wrapper">
                        ...
                        <table class="table table-bordered form_table">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="hidden" name="detail_id[]" value="{{$productreq->id}}">
                                    <select name="product_id[]" class="form-control product  product_select2" onchange="changeProduct(this)">
                                        <option value="">-SELECT-</option>
                                        @foreach($products as $product)
                                            <option value="{{$product->id}}" data-unitprice="{{$product->price}}" {{$productreq->product_id==$product->id?'selected':''}}>{{$product->name}}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td><input type="number" min="1" value="{{$productreq->unit_price}}"  step="any" name="price[]" class="form-control price" oninput="changePrice(this)"></td>
                                <td><input type="number" min="0" value="{{$productreq->quantity}}"  step="any" name="quantity[]" class="form-control quantity" oninput="changeQty(this)"></td>
                                <td><span class="total">{{$productreq->total_amount}}</span></td>
                                <td>
                                    @if(!$loop->first)
                            ...
                            <tr>
                                ...
                                <td class="totalamount">{{$requisition->details->sum('total_amount')}}</td>
                            </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div class="col-md-6 d-flex justify-content-end">
                    <button  class="btn w-25 rsbtn-1 bg-success" type="submit" >Update</button>
                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            /*$('.product').on('change',function(){
                let unnitprice=$('option:selected',this).attr('data-unitprice');
                let qty=$(this).parent().parent().find('.quantity').val(1);
                $(this).parent().parent().find('.price').val(unnitprice);
                $(this).parent().parent().find('.total').html(unnitprice);
                totalamount();
            })

            $('.price').on('input',function(){
                let unnitprice=$(this).val();
                let qut=$(this).parent().parent().find('.quantity').val();
            ...
            $('.quantity').on('input',function(){
                let qty=$(this).val();
                let unnitprice=$(this).parent().parent().find('.price').val();
